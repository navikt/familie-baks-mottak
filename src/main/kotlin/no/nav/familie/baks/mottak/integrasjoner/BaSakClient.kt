...
class BaSakClient
    ...
    constructor(
        ...
        private val authorizedRestClient: AuthorizedRestClient,
    ) {
        fun sendTilSak(nyBehandling: NyBehandling) {
            val uri = URI.create("$sakServiceUri/behandlinger")
            logger.info("Sender søknad til {}", uri)
            try {
                val response = authorizedRestClient.put(uri, nyBehandling, baSakScope, object : ParameterizedTypeReference<Ressurs<String>>() {})
        ...
        fun sendSvalbardtilleggTilBaSak(personIdent: String) {
            ...
            try {
                val response = authorizedRestClient.post(uri, PersonIdent(ident = personIdent), baSakScope, object : ParameterizedTypeReference<Ressurs<String>>() {})
        ...
        fun hentFagsaknummerPåPersonident(personIdent: String): Long {
            ...
            return runCatching {
                authorizedRestClient.post(uri, mapOf("personIdent" to personIdent), baSakScope, object : ParameterizedTypeReference<Ressurs<RestFagsakId>>() {})
            }.fold(
        ...
        fun hentRestFagsakDeltagerListe(
            ...
            return runCatching {
                authorizedRestClient.post(uri, RestSøkParam(personIdent, barnasIdenter), baSakScope, object : ParameterizedTypeReference<Ressurs<List<RestFagsakDeltager>>>() {})
            }.fold(
        ...
        fun hentFagsakerHvorPersonErSøkerEllerMottarOrdinærBarnetrygd(
            ...
            return runCatching {
                authorizedRestClient.post(uri, RestPersonIdent(personIdent), baSakScope, object : ParameterizedTypeReference<Ressurs<List<RestFagsakIdOgTilknyttetAktørId>>>() {})
            }.fold(
                onSuccess = { it.data ?: throw IntegrasjonException(it.melding, null, uri, personIdent) },
        ...
        fun hentMinimalRestFagsak(fagsakId: Long): RestMinimalFagsak {
            ...
            return runCatching {
                authorizedRestClient.get(uri, baSakScope, object : ParameterizedTypeReference<Ressurs<RestMinimalFagsak>>() {})
            }.fold(
        ...
        fun hentRestFagsak(fagsakId: Long): RestFagsak {
            ...
            return runCatching {
                authorizedRestClient.get(uri, baSakScope, object : ParameterizedTypeReference<Ressurs<RestFagsak>>() {})
            }.fold(
                onSuccess = { it.data ?: throw IntegrasjonException(it.melding, null, uri) },
                onFailure = { throw IntegrasjonException("Feil ved henting av RestFagsak fra ba-sak.", it, uri) },
            )
