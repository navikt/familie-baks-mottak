...
class DokarkivClientTest : AbstractWiremockTest() {
    @Autowired
    ...
    fun `oppdaterJournalpost skal kjøre OK`() {
        stubFor(
            put(urlEqualTo("/api/arkiv/v2/${jp.journalpostId}"))
    ...
    fun `ferdigstillJournalpost skal kjøre OK`() {
        ...
        stubFor(
            put(urlEqualTo("/api/arkiv/v2/$journalpostId/ferdigstill?journalfoerendeEnhet=9999"))
                .willReturn(
                    aResponse()
                        .withHeader("Content-Type", "application/json")
                        .withBody(response),
