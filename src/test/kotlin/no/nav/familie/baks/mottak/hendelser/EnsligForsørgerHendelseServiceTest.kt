...
class EnsligForsørgerHendelseServiceTest {
    ...
    fun `Skal gjøre kall mot ba-sak hvis det er en overgangstønad som ikke er prosessert før`() {
        service.prosesserEfVedtakHendelse(
            42,
            EnsligForsørgerVedtakhendelse(100, "01020300110", StønadType.OVERGANGSSTØNAD)
        )

        verify(exactly = 1) {
            mockBaSakClient.sendVedtakOmOvergangsstønadHendelseTilSak("01020300110")
        }
    ...
    fun `Skal ignorere hendelse fra ef hvis alt lest`() {
        every {
            mockHendelsesloggRepository.existsByHendelseIdAndConsumer(
                "200",
        ...
        service.prosesserEfVedtakHendelse(
            42,
            EnsligForsørgerVedtakhendelse(200, "01020300110", StønadType.OVERGANGSSTØNAD)
        )
